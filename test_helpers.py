...
class BETestCase(TestCase):
    ...
    def assertQuerySetEqualByPK(self, qs1, qs2):
        self.assertEqual([i.pk for i in qs1], [i.pk for i in qs2])
...
class Factory(DjangoFunctionalFactory):
    ...
    @classmethod
    def user(cls, with_site_and_portfolios=False):
        email = cls.rand_email()
        u = User.objects.create(username=email, email=email)
        p = cls.rand_str(length=20)
        u.is_active = True
        u.set_password(p)
        u.save()

        if with_site_and_portfolios:
            num_portfolios = cls.rand_int(end=3)

            for p_count in range(0, num_portfolios):
                num_sites = cls.rand_int(end=5)
                p = cls.portfolio()
                cls.principalportfoliorelation(user=u, portfolio=p)
                for s_count in range(0, num_sites):
                    cls.site(portfolio=p)
        return u, p
    ...
    def site(cls, portfolio=None, floor_area="undefined", state="undefined", **kwargs):
        ...
        # Needed instead of None so that we can explicitly pass null values into the constructor.
        if floor_area == "undefined":
            floor_area = cls.rand_int(start=5, end=10000)

        if state == "undefined":
            state = cls.rand_us_state()

        site_kwargs = {
            "name": cls.rand_str(length=50),
            "address1": cls.rand_street_address(),
            "address2": "Suite %s" % cls.rand_int(),
            "city": cls.rand_city(),
            "state": state,
            "zipcode": cls.rand_int(start=10000, end=99999),
            # "latitude": cls.rand_float(-90, 90),
            # "longitude": cls.rand_float(0, 180),
            "phone_number": cls.rand_phone(),
            "meta_data": {"area": floor_area}
        }
        site_kwargs.update(kwargs)

        s = Site.objects.create(**site_kwargs)

        s.portfolios.add(portfolio)
        s.save()
        return s

    @classmethod
    @classmethod
    def principalsiterelation(cls, user=None, portfolio=None, site=None):
        if not user:
            user = cls.user()
        ...
        return PrincipalSiteRelation.objects.create(
                user=user,
                portfolio=portfolio,
                site=site,
            )
    ...
    @classmethod
    def csv_to_import(cls, name=None, num_rows=None, column_headers=None, data_rows=None):
        if not num_rows:
            if data_rows:
                num_rows = len(data_rows)
            else:
                num_rows = cls.rand_int(start=2, end=10)
        if not column_headers:
            column_headers = [cls.rand_str() for i in range(0, cls.rand_int(end=20))]
        ...
        with open(f.name, 'wb') as csvfile:
            ...
            if data_rows:
                ...
                for i in range(0, num_rows):
                    row = ["%s-%s" % (i, cls.rand_int()) for h in range(0, len(column_headers))]
                    csv_writer.writerow(row)

        return f

    @classmethod
    def import_file(cls, csv_to_import=None, import_record=None, num_rows=None, column_headers=None, **kwargs):
        if not import_record:
            import_record = cls.import_record(num_files=0)

        if not csv_to_import:
            csv_to_import = cls.csv_to_import(num_rows=num_rows, column_headers=column_headers)

        import_file = ImportFile.objects.create(
                import_record=import_record,
            )
        import_file.file = File(open(csv_to_import.name))
        import_file.save()
    ...
    def import_record(cls, userprofile=None, num_files=None, **kwargs):
        ...
        for i in range(0, num_files):
            cls.import_file(import_record=record)
        return record
