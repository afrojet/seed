...
from mcm.mappings import espm as espm_mapping
...
from seed.models import (
    ...
    PORTFOLIO_BS,
    get_column_mappings,
    find_unmatched_building_values,
...
def filter_concat_configs(mappings):
    """Pull out concat configurations out of mappings.

    :param mappings: dict, key is target/destination, value is raw.

    Iff raw value is json decodeable and is a list, then a concat config
    is created for that "row".

    :return type tuple: (dict, list). Remaining mappings and list of concat
    ...
    for value, target in mappings.items():
        try:
            # This is kind of sub optimal, but necessary for backwards compat.
            v = json.loads(value)
...
def _save_raw_data(file_pk, *args, **kwargs):
    ...
    tasks = add_cache_increment_parameter(tasks)
    import_file.num_columns = len(parser.csvreader.unicode_fieldnames)
    import_file.save()
