...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'BuildingSnapshot.import_record'
        db.delete_column(u'seed_buildingsnapshot', 'import_record_id')

        # Adding M2M table for field seed_org on 'BuildingSnapshot'
        m2m_table_name = db.shorten_name(u'seed_buildingsnapshot_seed_org')
        db.create_table(m2m_table_name, (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('buildingsnapshot', models.ForeignKey(orm[u'seed.buildingsnapshot'], null=False)),
            ('organization', models.ForeignKey(orm[u'organizations.organization'], null=False))
        ))
        db.create_unique(m2m_table_name, ['buildingsnapshot_id', 'organization_id'])


    def backwards(self, orm):
        # Adding field 'BuildingSnapshot.import_record'
        db.add_column(u'seed_buildingsnapshot', 'import_record',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['data_importer.ImportRecord'], null=True, blank=True),
                      keep_default=False)

        # Removing M2M table for field seed_org on 'BuildingSnapshot'
        db.delete_table(db.shorten_name(u'seed_buildingsnapshot_seed_org'))
