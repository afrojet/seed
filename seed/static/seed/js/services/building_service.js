...
.factory('building_services', [
  ...
  'user_service',
  function ($http, $q, urls, label_helper_service, user_service) {

    var building_factory = {};

    building_factory.get_buidlings_for_user = function() {
        // django uses request.user for user information
        var defer = $q.defer();
        $http({
            method: 'GET',
            'url': window.BE.urls.get_buildings_for_user_url
    ...
    building_factory.get_building = function(building_id) {
        ...
        $http({
            ...
            for (var i = 0; i < data.compliance_projects.length; i++) {
                var building = data.compliance_projects[i].building;
    ...
    building_factory.search_PM_buildings = function(query_string, number_per_page, page_number, order_by, sort_reverse, filter_params, import_file_id, project_id, project_slug) {
        var defer = $q.defer();
        $http({
            'method': 'POST',
            'data': {
    ...
    building_factory.get_PM_building = function(building_id) {
        var defer = $q.defer();
        $http({
            'method': 'POST',
            'data': {
    ...
    building_factory.get_PM_building_matches = function(building_id, import_record_id) {
        var defer = $q.defer();
        $http({
            'method': 'POST',
            'data': {
            ...
            'url': urls.get_PM_building_matches
        }).success(function(data, status, headers, config){
            defer.resolve(data);
        }).error(function(data, status, headers, config){
            defer.reject(data, status);
    ...
    building_factory.get_columns = function(is_project, project_slug) {
        var defer = $q.defer();
        $http({
            method: 'GET',
