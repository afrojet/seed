...
describe("controller: dataset_detail_controller", function(){
    ...
    it("should filter import files by source types: 'Assessed Raw' and 'Portfolio Raw'",
        function() {
        // arrange
        create_dataset_detail_controller();
        ...
        // assertions
        expect(dataset_detail_ctrl_scope.dataset.importfiles[0].source_type)
            .toBe('Assessed Raw');
        expect(dataset_detail_ctrl_scope.dataset.importfiles[1].source_type)
            .toBe('Portfolio Raw');

        var importfiles = dataset_detail_ctrl_scope.dataset.importfiles;
        // filter importfiles by 'Portfolio Raw'
        expect(ngFilter('filter')(importfiles,
            dataset_detail_ctrl_scope.dataset_filter.pm)[0])
            .toBe(importfiles[1]);
        // filter importfiles by 'Assessed Raw'
        expect(ngFilter('filter')(importfiles,
            dataset_detail_ctrl_scope.dataset_filter.assessor)[0])
            .toBe(importfiles[0]);
    });
