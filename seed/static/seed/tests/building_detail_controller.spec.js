...
describe("controller: building_detail_controller", function(){
    ...
    it("should show the default projects table if a user has no compliance" +
        ...
        building_detail_ctrl_scope.compliance_projects = [{id: 1, name: "a"}];
    ...
    it("should display all the extra data within all the buildings", function() {
        // arrange
        create_building_detail_controller();
        
        // act
        building_detail_ctrl_scope.$digest();
        building_detail_ctrl_scope.generate_extra_data_columns(
            [building_detail_ctrl_scope.building].concat(building_detail_ctrl_scope.imported_buildings)
        );
        building_detail_ctrl_scope.$digest();

        // assertions
        var edc = building_detail_ctrl_scope.extra_data_columns;

        // should not duplicate keys
        expect(edc.length).toEqual(4);
        expect(edc.indexOf('make it four')).toEqual(3);
    });
